    ...
    def makePlot(self, pathAcc, pathLoss):
        ...
        for s in range(len(seriesX)):
            ...
            if(types[s]=="Top" or types[s]=="Bot" or types[s]=="Random"):
                ...
                plt.plot(seriesX[s], seriesY[s],label='_nolegend_',linestyle=cases[mapping+3] ,linewidth=1, color=cases[mapping+1])
            else:
                print(types[s])
                plt.scatter(seriesX[s], seriesY[s],s=10, marker=cases[mapping+2],color=cases[mapping+1])
                plt.plot(seriesX[s], seriesY[s],label=cases[mapping] ,linestyle=cases[mapping+3] ,linewidth=1, color=cases[mapping+1])
