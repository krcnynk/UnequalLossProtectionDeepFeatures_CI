...
from errConceal.fantec import *
from errConceal.bilinear import *
import cv2 as cv2
...
def fnRunImgClassDemo(modelDict,splitLayerDict,ecDict,batch_size,path_base,transDict,outputDir):
    ...
    for i_b in range(len(batched_y_labels)):
        ...
        if channel_flag in ['EX','RL','GC']:
            ...
            inpaint_masks_list = []
            pkt_obj_list_fantec = []
            pkt_obj_list_bilinear = []

            print("len(deviceOut) is %d" % len(deviceOut))
            print("MIN",np.min(deviceOut[i]))
            for i in range(len(deviceOut)):
                ...
                pkt_obj_list_fantec.append(PacketModel(rows_per_packet=rowsPerPacket,data_tensor=np.copy(deviceOut[i])))
                pkt_obj_list_bilinear.append(PacketModel(rows_per_packet=rowsPerPacket,data_tensor=np.copy(deviceOut[i])))
            # ---------------------------------------------------------------- #
                ...
                for item_index in range(np.shape(loss_map)[0]):
                    ...
                    if len(lost_pkt_indices) != 0:
                        ...
                        for k in range(len(lost_pkt_indices)):
                            ...
                            pkt_obj_list_fantec[i].packet_seq[item_index,lost_pkt_indices[k],:,:,lost_channel_indices[k]] = np.zeros([rowsPerPacket,channel_width])
                            pkt_obj_list_bilinear[i].packet_seq[item_index,lost_pkt_indices[k],:,:,lost_channel_indices[k]] = np.zeros([rowsPerPacket,channel_width])
            # ---------------------------------------------------------------- #
            # TODO: Add fantec/bilinear neighbour tensor concealement
            # Error loss concealment.
            print('Running packet loss concealment.')
            for i in range(len(pkt_obj_list_caltec)):
                # print(loss_map)
                # print("len(pkt_obj_list_caltec) is %d" % len(pkt_obj_list_caltec))
                # print("Loss map shape is {}".format(loss_map.shape))
                # ALTeC
                ...
                # fantec
                repaired_tensor = fn_fantec(np.copy(pkt_obj_list_fantec[i].data_tensor),rowsPerPacket,loss_map)
                pkt_obj_list_fantec[i].data_tensor = repaired_tensor
                # bilinear
                repaired_tensor = fn_bilinear(np.copy(pkt_obj_list_bilinear[i].data_tensor),rowsPerPacket,loss_map)
                pkt_obj_list_bilinear[i].data_tensor = repaired_tensor   
            ...
            if quant_tensor1 != 'noQuant':
                ...
                quant_tensor1.bitQuantizer(np.copy(pkt_obj_list_fantec[0].data_tensor))
                fantec_quant.append(quant_tensor1.quanData)
                quant_tensor1.bitQuantizer(np.copy(pkt_obj_list_bilinear[0].data_tensor))
                bilinear_quant.append(quant_tensor1.quanData)
                print("min ",np.min(bilinear_quant[0]))
            # Save quantized tensors as image.
            for i in range(len(deviceOut)):
                ...
                quant_fantec = fantec_quant[i]
                quant_bilinear = bilinear_quant[i]
                ...
                for item_index in range(np.shape(quant_caltec)[0]):
                    for i_c in range(np.shape(quant_caltec)[-1]):
                        ...
                        tensor_channel = Image.fromarray(quant_fantec[item_index,:,:,i_c].astype(np.uint8))
                        tensor_channel.save(os.path.join(results_dir,'Fantec_batch_'+str(i_b)+'_item_'+str(item_index)+'_tensor_'+str(i)+'_channel_'+str(i_c)+res_filename+'.png'))
                        tensor_channel = Image.fromarray(quant_bilinear[item_index,:,:,i_c].astype(np.uint8))
                        print("min2 ",np.min(quant_bilinear[item_index,:,:,i_c].astype(np.uint8)))
                        tensor_channel.save(os.path.join(results_dir,'Bilinear_batch_'+str(i_b)+'_item_'+str(item_index)+'_tensor_'+str(i)+'_channel_'+str(i_c)+res_filename+'.png'))
            # ---------------------------------------------------------------- #
            caltec_tensor_list = []
            altec_tensor_list = []
            halrtc_tensor_list = []
            ...
            for i in range(len(pkt_obj_list_caltec)):
                ...
                fantec_tensor_list.append(pkt_obj_list_fantec[i].data_tensor)
            ...
            fantec_out = cloud_model.predict(fantec_tensor_list)
            fantec_Top1_pred = np.argmax(fantec_out,axis=1)
            fantec_Top1_confidence = np.max(fantec_out,axis=1)
            top1_conf_fantec.extend(fantec_Top1_confidence)
            top1_pred_fantec.extend(fantec_Top1_pred)

            for i_item in range(np.shape(fantec_out)[0]):
                item_top5_predictions = np.argpartition(-fantec_out[i_item,:],5)[:5]
                top5_pred_fantec.append(item_top5_predictions)
    ...
    if channel_flag in ['GC','RL','EX']:
        ...
        f.write('\n Top-1 Fantec predictions \n')
        f.write(str(top1_pred_fantec))
        f.write('\n Top-1 Fantec confidence \n')
        f.write(str(top1_conf_fantec))
        f.write('\n Top-5 Fantec predictions \n')
        f.write(str(top5_pred_fantec))
