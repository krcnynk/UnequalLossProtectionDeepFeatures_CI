...
def fnRunImgClassMC(modelDict,splitLayerDict,ecDict,MC_runs,mc_task,batch_size,path_base,transDict,outputDir):
    ...
    if 'GilbertChannel' in channel:
        print("Korcan-This is a GC Channel")
    ...
    if ecDict != 'noEC':
        print("Korcan-There is error concealement")
        ec_method = [*ecDict][0]
        if ec_method == 'ALTeC':
            print("Korcan-First EC is ALTEC FOUND")
    ...
    # print(np.asarray(dataset_x_files).shape)
    # print(len(dataset_x_files))
    ...
    if mc_task == 'LoadLossPatterns':
        print("Korcan-This is LoadLossPatterns")
        if channel_flag == 'EX':
            loss_matrix_mc = []
